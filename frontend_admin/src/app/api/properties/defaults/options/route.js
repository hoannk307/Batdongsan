  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        data || { message: "Failed to fetch defaults options" },
        { status: res.status }
      );
    }
    ...
    if (isConnectionError(e)) {
      console.warn(`[API/properties/defaults/options] Backend không khả dụng tại ${targetUrl}`);
      return NextResponse.json(
        { message: "Backend không khả dụng. Vui lòng thử lại sau." },
        { status: 503 }
      );
    }
    console.error("[API/properties/defaults/options] Lỗi không xác định:", e);
    return NextResponse.json(
